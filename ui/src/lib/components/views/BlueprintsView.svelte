...
<div class="p-6 md:p-10 max-w-7xl mx-auto w-full">
	<div class="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
		<div>
			<h1 class="text-2xl font-semibold text-gray-900 tracking-tight">Hosted Projects</h1>
			<p class="text-gray-500 mt-1">Manage your deployed applications and services.</p>
		</div>
		<button
			type="button"
	...
	{#if blueprints.length === 0}
		<div class="bg-white border border-gray-200 rounded-xl p-12 text-center text-gray-500 shadow-sm">
			...
			<h3 class="text-base font-semibold text-gray-900 mb-1">No Projects Configured</h3>
			<p class="text-sm text-gray-500 max-w-md mx-auto mb-6">Create or deploy applications from a repository or scratch.</p>
			<button
				type="button"
				onclick={onCreateBlueprint}
	...
	{:else}
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
			{#each blueprints as bp}
				<div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex flex-col hover:border-blue-300 hover:shadow-md transition-all group cursor-pointer" onclick={() => window.location.assign(`/projects/${bp.id}`)} onkeydown={(e) => e.key === 'Enter' && window.location.assign(`/projects/${bp.id}`)} tabindex="0" role="button">
					...
					<p class="text-sm text-gray-500 mb-4 flex-1">
						{bp.service_count_actual || bp.serviceCount || 0} Service(s) active.
					</p>
					<div class="pt-4 border-t border-gray-100 flex items-center justify-between text-sm">
						<span class="text-gray-400 flex items-center gap-1.5 font-mono text-xs truncate max-w-[130px]" title={bp.repo_url || 'No Repository'}>
							<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
							{bp.repo_url || 'No Repository'}
						</span>
						<div class="flex items-center gap-3">
							<button
								type="button"
								onclick={(e) => { e.stopPropagation(); onPromptDeleteBlueprint(bp); }}
