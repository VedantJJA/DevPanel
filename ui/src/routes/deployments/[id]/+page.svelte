<script lang="ts">
	...
	let activeTab = $state<'logs' | 'environment' | 'settings' | 'metrics'>('logs');
	...
	<header class="border-b border-gray-200 bg-white py-4 px-6 md:px-10 flex items-center justify-between shadow-sm">
		<div class="flex items-center gap-3">
			...
			<div>
				...
				<p class="text-xs text-gray-500 font-mono mt-0.5">{projectData?.blueprint.repo_url || 'Deployment Pipeline'}</p>
			</div>
		</div>
		...
		{#if projectData}
			...
			<div class="flex border-b border-gray-200 gap-6 text-sm font-medium text-gray-500">
				<button
					...
					class="py-3 border-b-2 transition-colors flex items-center gap-2 {activeTab === 'logs' ? 'border-blue-600 text-blue-600 font-semibold' : 'border-transparent hover:text-gray-900'}"
				>
					<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
					<span>Build Logs Stream</span>
				</button>

				<button
					type="button"
					onclick={() => (activeTab = 'environment')}
					class="py-3 border-b-2 transition-colors flex items-center gap-2 {activeTab === 'environment' ? 'border-blue-600 text-blue-600 font-semibold' : 'border-transparent hover:text-gray-900'}"
				>
				...
				<button
					...
					class="py-3 border-b-2 transition-colors flex items-center gap-2 {activeTab === 'metrics' ? 'border-blue-600 text-blue-600 font-semibold' : 'border-transparent hover:text-gray-900'}"
				>
				...
				<button
					...
					class="py-3 border-b-2 transition-colors flex items-center gap-2 {activeTab === 'settings' ? 'border-blue-600 text-blue-600 font-semibold' : 'border-transparent hover:text-gray-900'}"
				>
			...
			{#if activeTab === 'logs'}
				<div class="h-[550px]">
					<Terminal {projectId} serviceFilter={currentSvc?.name} title={`Live Deployment Build Stream: ${currentSvc?.name || projectId}`} />
				</div>
			{:else if activeTab === 'environment'}
				<div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-6">
					...
					<div class="flex justify-end">
						...
						>
							{isSaving ? 'Saving Environment...' : 'Save Environment Variables'}
						</button>
					</div>
				</div>
			...
			{:else if activeTab === 'settings'}
				<div class="bg-white border border-gray-200 rounded-xl p-6 md:p-8 shadow-sm space-y-6 max-w-3xl">
					<div>
						...
						<p class="text-sm text-gray-500 mt-1">Edit commands and container target port.</p>
					</div>
