...
<div class="min-h-screen bg-neutral-950 text-neutral-100 p-6 md:p-12 font-sans antialiased flex flex-col items-center justify-center">
	<div class="w-full max-w-2xl space-y-8">
		...
		{#if alertState}
			<div
				class="p-4 rounded-2xl border transition-all text-sm shadow-xl flex items-start gap-3.5 {alertState.type === 'error'
			...
			>
				...
				<div class="space-y-2 flex-1">
					<p class="font-medium leading-relaxed">{alertState.message}</p>

					{#if alertState.blueprint}
						<div class="mt-3 p-3 rounded-xl bg-neutral-900/90 border border-neutral-800 text-xs font-mono space-y-1.5 text-neutral-300">
		...
		>
			...
			<div class="space-y-2">
				...
				<input
					...
					type="text"
					bind:value={appName}
					placeholder="e.g., my-monorepo-startup"
					disabled={loading}
					class="w-full px-4 py-3 bg-neutral-950 border border-neutral-800 rounded-xl text-sm text-neutral-100 placeholder-neutral-500 focus:outline-none focus:border-emerald-500/60 focus:ring-1 focus:ring-emerald-500/30 transition-all disabled:opacity-50"
					required
				/>
			</div>
			...
			<div class="space-y-2">
				...
				<div class="relative">
					<input
						...
						disabled={loading}
						class="w-full pl-11 pr-4 py-3 bg-neutral-950 border border-neutral-800 rounded-xl text-sm text-neutral-100 placeholder-neutral-500 focus:outline-none focus:border-emerald-500/60 focus:ring-1 focus:ring-emerald-500/30 transition-all disabled:opacity-50"
						required
					/>
