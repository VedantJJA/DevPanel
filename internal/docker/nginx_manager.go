...
func (m *NginxManager) writeMappingsAndReload(mappings map[string]string) error {
	...
	sb.WriteString("    listen [::]:80 default_server;\n")
	sb.WriteString("    server_name _ *.klouds.online klouds.online;\n\n")
	...
	_ = os.WriteFile(nginxTempPath, []byte(sb.String()), 0644)
	_ = os.Rename(nginxTempPath, nginxConfigPath)

	// 2. Also attempt overwriting /etc/nginx/sites-available/default if present
	defaultSitePath := "/etc/nginx/sites-available/default"
	if _, err := os.Stat(defaultSitePath); err == nil {
		_ = os.WriteFile(defaultSitePath, []byte(sb.String()), 0644)
	}
